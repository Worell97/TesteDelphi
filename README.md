...
Ok (No fechamento do formulário não consegui encerrar as threads 'gentilmente')
...
Solução:
    Só usar o next caso o registro não seja deletado.
    Erro: Ao remover o registro selecionado do dataset o proximo registro é automaticamente selecionado, sendo assim não se deve usar o next quando o registro for deletado, apenas se o mesmo não for par.
...
Solução:
    Salvar no dataset apenas o caminho do arquivo de origem.
    Em salvar arquivos, percorrer o data set, carregar cada arquivo em um TMemoryStream, e salvar na pasta "\Servidor"
...
Solução:
    Em "Enviar com erros" foi adicionado um bloco try/except onde ao ocorrer qualquer exceção é chamada a procedure RollBackOperation que simplesmente exclui todos os aquivos da pasta de destino.